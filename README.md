...
Click Launch instance
    ...
    Click launch 
        ...
        For the Key Pair, add what you have created or been given, in my case it was DevOpsStudents.pem
...
To copy files across to the WebApp 
    ...
    Scp moves the file into the virtual machine. Sinking OS to app folder in VM
        ...
        scp -i ~/.ssh/DevOpsStudents.pem -r ~/Documents/aws/app/ ubuntu@WEBAPP IP HERE:/home/ubuntu/

    Syncing provision folder to vm
    
        scp -i ~/.ssh/DevOpsStudents.pem -r ~/Documents/aws/environment/app/provision.sh ubuntu@WEBAPP IP HERE:/home/ubuntu/
...
Once this is done, ssh into the Bastion server 
    ...
    ssh -i ~/.ssh/DevOpsStudents.pem ubuntu@BASTION IP GOES HERE
...
Now ssh to the Database from the bastion using
    ...
    ssh -i ~/.ssh/DevOpsStudents.pem ubuntu@DATABASE IP GOES HERE  
